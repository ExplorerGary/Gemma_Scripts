...
fieldnames = ["bucket_name","gamma","beta","mu"]
gemma_table = None
r_gemma_table = None
# 0. 准备工具函数：
...
# --- hook本体 ---
def cal_parameters_hook(
    process_group: dist.ProcessGroup, bucket: dist.GradBucket
) -> torch.futures.Future[torch.Tensor]:

    # --- 导入东西 --- 
    global CURRENT_EPOCH,CURRENT_STEP,save_Bucket,Scaling,Pioneer,OUTPUT_DIR
    # --- 缓冲区里的扁平向量 --- 
    ...
    distribution = cal_distribution(flat_tensor) if Scaling is None else cal_distribution(quantized)
    distribution["bucket_name"] = file_name[:-3]
    csv_path = os.path.join(OUTPUT_DIR,f"003_BUCKET_PARAMETERS_rank{rank}.csv")
    ...
    def training_step(
        ...
        if dist.is_initialized() and self.hook_registered == False:
            try:
                ...
                model.register_comm_hook(state=None, hook=cal_parameters_hook)
